...
def build_reconciliation_law(t):
    if t == 3:
        ...
        raise ValueError("不支持的 t 值用于归整。请选择 3, 7, 或 13。")
...
for name, params in lore_params.items():
    ...
    if sum(s_dist_counts.values()) != params["n_ring"]:
        raise ValueError(f"自定义私钥系数总和 {sum(s_dist_counts.values())} 与 n_ring {params['n_ring']} 不匹配!")
    ...
    print(f"     Custom counts per poly: -2: {s_dist_counts[-2]}, -1: {s_dist_counts[-1]}, 0: {s_dist_counts[0]}, 1: {s_dist_counts[1]}, 2: {s_dist_counts[2]}")
